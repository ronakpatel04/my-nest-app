...
export class DocumentController {
    ...
    async uploadDocument(@UploadedFile() file, @Req() req: AuthenticatedRequest) {
      try {
        if (!file) return createResponse(false, 400, 'File is required');
  
        if (!req.user) {
            throw new UnauthorizedException('User information is missing in the request');
          }
        const user = req.user; 
    ...
    @Roles('ADMIN', 'EDITOR')
    @Patch(':id')
    async updateDocument(@Param('id') id: string, @Body() body, @Req() req: AuthenticatedRequest) {
      const user = req.user; 
      if(!user)
      {
        throw new UnauthorizedException('User information is missing in the request');
